...
class TestingLocalHostSupabaseUser extends BaseAuthUser {
  TestingLocalHostSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> testingLocalHostSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TestingLocalHostSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
